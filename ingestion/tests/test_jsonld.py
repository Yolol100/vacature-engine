...
class JsonLdTests(unittest.TestCase):
    def test_extract_jobposting(self):
        ...
        self.assertEqual(row["source_metadata"]["metadata_extractor"], "extruct-0.18.0")

    def test_trafilatura_fallback_when_jobposting_description_is_missing(self):
        url = "https://example.test/jobs/wp"
        html = '''
        <html><body>
          <main><h1>WordPress Engineer</h1><p>Build and maintain WooCommerce sites for remote clients.</p></main>
          <script data-source="careers" type="application/ld+json">
          {"@type":"JobPosting","title":"WordPress Engineer","identifier":{"value":"wp-2"},"hiringOrganization":{"name":"Acme"},"url":"https://example.test/jobs/wp"}
          </script>
        </body></html>
        '''
        spec = SourceSpec("company-acme", "employer_direct", "jsonld", "acme", options={"urls": [url]})
        adapter = ADAPTERS["jsonld"]
        row = adapter.normalize_records(adapter.fetch(FakeClient({url: html}), spec), spec)[0]
        self.assertIn("WooCommerce", row["description"])
        self.assertEqual(row["source_metadata"]["main_text_fallback"], "trafilatura-2.2.0")
