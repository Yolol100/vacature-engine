...
from vacature_engine.policy import MATCH_COMPONENTS, OPPORTUNITY_COMPONENTS, application_guard, hard_gate, score  # noqa: E402
...
def run_audit() -> dict[str, object]:
    ...
    blocking = (
        "mandatory_relocation", "structural_office_attendance", "unpaid_test", "commission_only",
        "suspicious_payment", "marketplace_excluded", "duplicate", "stale_repost",
        "geographic_restriction_blocks", "us_residents_only",
    )
    for bits in itertools.product((False, True), repeat=len(blocking)):
        data = good_gate(**dict(zip(blocking, bits)))
    ...
    actives = (True, False)
    links = (True, False)
    levels: tuple[object, ...] = ("Senior Engineer", "Medior Developer", "Junior Developer", "Software Engineer", ["Senior"])
    for age, eligibility, remote, active, link, level in itertools.product(ages, eligibilities, remotes, actives, links, levels):
        result = hard_gate(good_gate(posted_age_days=age, work_eligibility=eligibility, fully_remote=remote, active=active, official_link_working=link, level=level))
        age_ok = isinstance(age, (int, float)) and not isinstance(age, bool) and math.isfinite(float(age)) and 0 <= age <= 7
        level_ok = isinstance(level, str) and ("senior" in level.lower() or "medior" in level.lower()) and "junior" not in level.lower()
        expected = age_ok and eligibility in {"allowed", "plausible"} and remote and active and link and level_ok
    ...
    for value in (None, True, 1, ["Senior"], {"level": "Senior"}):
        ...
        audit.check(f"level_type_{value!r}", result["pass"] is False and "invalid_level" in result["reasons"])
    ...
    for bits in itertools.product((0, 1), repeat=len(score_keys)):
        data = perfect_score()
        for key, bit in zip(score_keys, bits):
            data[key] = maxima[key] if bit else 0
        result = score(data)
        expected_match = sum(float(data[k]) for k in MATCH_COMPONENTS)
        expected_opp = expected_match * 0.55 + sum(float(data[k]) for k in OPPORTUNITY_COMPONENTS)
    ...
    audit.check("score_cap_59", score(perfect_score(central_hard_missing=True))["match_score"] == 59.0)
    audit.check("score_multiple_excluded", score(perfect_score(multiple_central_hard_mismatches=True))["excluded"] is True)

    prepare_keys = (
        "user_explicitly_requested", "final_verification_pass", "hard_gate_pass", "cv_selected",
        "work_eligibility_confirmed", "legitimacy_check_pass",
    )
    for bits in itertools.product((False, True), repeat=len(prepare_keys)):
        data = {"stage": "prepare", **dict(zip(prepare_keys, bits))}
        audit.check(f"prepare_{bits}", application_guard(data)["pass"] is all(bits))

    draft_keys = (
        "user_explicitly_requested", "final_verification_pass", "hard_gate_pass", "cv_selected",
        "work_eligibility_confirmed", "legitimacy_check_pass", "recipient_verified",
        "recipient_authorized_for_role", "motivation_qa_pass", "cv_attachment_ready",
        "subject_exact_vacancy_title",
    )
    for bits in itertools.product((False, True), repeat=len(draft_keys)):
        data = good_draft(**dict(zip(draft_keys, bits)))
    ...
    for index in range(30):
        audit.check(f"hash_material_{index}", content_hash(f"Senior WordPress Developer requirement {index}") != reference)
    ...
    for index, url in enumerate((
        "http://127.0.0.1/", "http://10.0.0.1/", "http://169.254.169.254/", "http://localhost/",
        "http://service.internal/", "http://intranet/", "https://example.com:8443/", "https://user:pass@example.com/",
    )):
        try:
