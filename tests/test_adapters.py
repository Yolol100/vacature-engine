...
class FakeClient:
    ...
    def get_json(self, url, allowed_hosts=None):
        ...
        if isinstance(self.json_payload, list) and self.json_payload and isinstance(self.json_payload[0], dict) and "content" in self.json_payload[0]:
...
class AdapterTests(unittest.TestCase):
    def test_registry_has_keyless_adapters(self):
        self.assertEqual(AdapterRegistry.available(), ["ashby", "greenhouse", "lever", "personio", "smartrecruiters"])

    def test_greenhouse_normalizes(self):
        client = FakeClient({"jobs": [{"id": 1, "title": "Engineer", "absolute_url": "https://boards.greenhouse.io/acme/jobs/1", "location": {"name": "Remote"}, "content": "<p>Hello</p>", "updated_at": "2026-08-24"}]})
    ...
    def test_lever_created_at_and_remote(self):
        client = FakeClient([{"id": "abc", "text": "Engineer", "hostedUrl": "https://jobs.lever.co/acme/abc", "applyUrl": "https://jobs.lever.co/acme/abc/apply", "createdAt": 1787616000000, "workplaceType": "remote", "categories": {"location": "Remote", "team": "Platform"}}])
    ...
    def test_ashby_normalizes(self):
        client = FakeClient({"jobs": [{"id": "1", "title": "Product Engineer", "jobUrl": "https://jobs.ashbyhq.com/acme/1", "applyUrl": "https://jobs.ashbyhq.com/acme/1/application", "isListed": True, "isRemote": True, "location": "Europe"}]})
