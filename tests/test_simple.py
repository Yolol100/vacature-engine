...
class RemoteFirstPolicyTests(unittest.TestCase):
    def test_policy_zero_age_means_unlimited(self):
        gate = eligibility(vacancy(posted_date="2025-01-01"), today=TODAY, policy=POLICY)
    ...
    def test_positive_age_limit_still_works(self):
        ...
        self.assertFalse(eligibility(item, today=TODAY, policy=limited)["pass"])

    def test_remote_remains_hard_gate(self):
        for item in [vacancy(fully_remote=False), vacancy(fully_remote=None)]:
            with self.subTest(item=item):
                self.assertIn("not_remote", eligibility(item, today=TODAY, policy=POLICY)["reasons"])
    ...
    def test_invalid_salary_remains_fail_closed(self):
    ...
    def test_malformed_and_future_dates_rejected(self):
        malformed = eligibility(vacancy(posted_date="2026-08-20Z"), today=TODAY, policy=POLICY)
        future = eligibility(vacancy(posted_date="2026-09-01"), today=TODAY, policy=POLICY)
        self.assertIn("date_invalid", malformed["reasons"])
        self.assertIn("future_date", future["reasons"])
