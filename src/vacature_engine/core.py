...
TRACKING = {"gclid","fbclid","msclkid","mc_cid","mc_eid","ref","referrer","source","src","campaign","tracking","trk","trackingid","tracking_id"}
...
def canonical_url(url: str) -> str:
    ...
    q = [(k,v) for k,v in parse_qsl(p.query, keep_blank_values=True) if not (k.lower().startswith("utm_") or k.lower() in TRACKING)]
...
def norm(value: str) -> str:
    ...
    if not value: raise ValueError("value cannot normalize to empty")
    return value
...
def content_hash(text: str) -> str:
    ...
    if not text: raise ValueError("content required")
